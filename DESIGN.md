...
**Логотипы клиентов — паттерн отображения**: логотипы приходят в разных цветах и стилях (тёмные вордмарки, цветные эмблемы-печати), поэтому показываем их на светлых «чипах» (`background: #f4f4f4`, `border-radius`, `padding`) — это единственный способ одинаково хорошо показать любой логотип на тёмном фоне сайта. Приглушённая непрозрачность (`opacity: 0.8`) в покое, на hover — `opacity: 1` + подъём (`translateY`) + золотое свечение `--glow-gold` (классы `.trust-row img` на главной, `.logo-grid img` на страницах направлений). Из 47 присланных логотипов 32 уже задействованы (блок «Нам доверяют» на главной + портфолио `/design`), 15 — в резерве на будущее (`assets/clients/`: appzone, gnature, shabani, it, moscow-calling, psp, kadk, peri, forward, avrora, bani-onlayn, sosnovyi-bor, tomatolorika, feliks, frikdens).
